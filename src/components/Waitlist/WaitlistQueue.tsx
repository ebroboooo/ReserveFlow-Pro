  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">Waitlist Queue Management</h2>
      ...
      {filteredWaitlist.length === 0 ? (
        <div className="glass-card p-12 text-center border-slate-800/60 rounded-2xl">
          ...
          <h4 className="text-sm font-bold text-slate-300">Waitlist is currently empty</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredWaitlist.map((item) => {
            ...
            return (
              <div key={item.id} className="glass-card p-5 rounded-2xl relative border-slate-850 flex flex-col justify-between hover:border-brand-500/30 transition-all">
                <div className="space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-850 pb-2">
                    <span className="text-[10px] font-bold text-brand-400 bg-brand-500/10 px-2 py-0.5 rounded-md uppercase tracking-wider">
                  ...
                  <div>
                    <h4 className="font-bold text-white text-sm">{cust?.fullName || 'Unknown Customer'}</h4>
                    <p className="text-xs text-slate-400 font-semibold mt-0.5">{srv?.name}</p>
                    <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
                      <span>Preferred Employee: {emp?.name || 'Any Staff'}</span>
                    </p>
                  </div>

                  {item.notes && (
                    <div className="bg-slate-950/40 p-2.5 rounded-xl border border-slate-850/40 text-[11px] text-slate-400 font-medium">
                      {item.notes}
                    </div>
                  )}
                </div>

                <div className="flex items-center justify-between gap-3 mt-5 pt-3 border-t border-slate-850">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="bg-brand-500/10 hover:bg-brand-500/20 text-brand-300 text-xs px-2.5 py-1.5 rounded-lg font-semibold flex items-center gap-1 border border-brand-500/20"
                    >
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-6 shadow-2xl relative animate-in zoom-in-95 duration-150">
            <h3 className="text-lg font-bold text-white mb-4">Add to Waitlist</h3>

            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">Customer</label>
                <select

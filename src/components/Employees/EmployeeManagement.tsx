...
export const EmployeeManagement: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (formData.id) {
      ...
      await dispatchAuditLog("Employee Configured", `Modified employee schedules & info for ${formData.name}`);
    } else {
      await db.employees.create(org, payload);
      await dispatchAuditLog("Employee Configured", `Registered new employee staff member: ${formData.name}`);
    }
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Are you sure you want to deactivate staff member: ${name}?`)) {
      ...
      await dispatchAuditLog("Employee Deactivated", `Deactivated employee: ${name}`);
      await refreshData();
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {employees.map(emp => {
          ...
          return (
            <div key={emp.id} className="glass-card p-5 rounded-2xl border-slate-850 flex flex-col justify-between hover:border-brand-500/30 transition-all group">
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-850 pb-3">
                  <div>
                    <h4 className="font-bold text-white text-base">{emp.name}</h4>
                    <span className="text-[10px] text-brand-400 font-semibold">{emp.role}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-950/40 p-3 rounded-xl border border-slate-850/40 text-center">
                  <div>
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Completed Sessions</p>
                    <p className="text-sm font-bold text-slate-200 mt-0.5">{metrics.completedCount}</p>
                  </div>
                  <div>
              ...
              <div className="flex items-center justify-between gap-3 mt-6 pt-3 border-t border-slate-850">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="p-1.5 bg-slate-850 hover:bg-slate-700/80 text-slate-300 hover:text-white rounded-lg text-xs"
                  >
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative animate-in zoom-in-95 duration-150 flex flex-col max-h-[90vh]">
            ...
            <div className="flex border-b border-slate-800 mb-4">
              <button
                ...
                className={`py-2 px-4 text-sm font-bold border-b-2 transition-all ${
                  activeTab === 'profile' ? 'border-brand-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
              >
                Profile Info
              </button>
              <button
                ...
                className={`py-2 px-4 text-sm font-bold border-b-2 transition-all ${
                  activeTab === 'schedule' ? 'border-brand-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
              >
            ...
            <form onSubmit={handleSave} className="space-y-4 flex-1 overflow-y-auto pr-1">
              {activeTab === 'profile' ? (
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">Employee Name</label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-2 max-h-24 overflow-y-auto border border-slate-850 p-2 rounded-xl">
                      {branches.map(b => (
                        <label key={b.id} className="flex items-center gap-2 text-xs font-semibold text-slate-300 select-none cursor-pointer">
                          <input
                            type="checkbox"
                            checked={formData.selectedBranches.includes(b.id)}
                            onChange={() => handleBranchToggle(b.id)}
                            className="rounded bg-slate-950 border-slate-800 text-brand-500 focus:ring-brand-500/30"
                          />
                          <span>{b.name}</span>
                        </label>
                      ))}
                ...
                <div className="space-y-3.5">
                  ...
                  {formData.schedule.map((day, idx) => (
                    <div key={idx} className="flex items-center justify-between gap-3 p-2 bg-slate-950/40 border border-slate-850/60 rounded-xl">
                      <label className="flex items-center gap-2 text-xs font-bold text-slate-300 min-w-[70px] select-none cursor-pointer">
                        <input
                          type="checkbox"
                      ...
                      {day.isWorking ? (
                        ...
                        <span className="text-[10px] text-slate-550 italic font-bold">Rest Day</span>
                      )}
                    </div>
                  ))}
                </div>

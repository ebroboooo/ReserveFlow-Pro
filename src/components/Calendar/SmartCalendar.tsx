...
export const SmartCalendar: React.FC = () => {
  ...
  const getStatusColor = (status: ReservationStatus) => {
    switch (status) {
      ...
      case 'Confirmed': return 'bg-brand-500/20 border-brand-500/40 text-brand-700';
      ...
      case 'NoShow': return 'bg-slate-500/10 border-slate-500/30 text-slate-700';
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-50 border border-slate-200 p-4 rounded-2xl">
        <div className="flex items-center gap-3">
          <button 
            onClick={handlePrev}
            className="p-2 bg-slate-100/60 border border-slate-300/80 rounded-xl text-slate-700 hover:text-slate-900"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span className="text-sm font-bold text-slate-900 uppercase tracking-wider min-w-[120px] text-center">
            {currentView === 'Week' 
            ...
            {(['Day', 'Week', 'Month'] as const).map(view => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg font-semibold transition-all ${
                  currentView === view ? 'bg-brand-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-800'
                }`}
              >
                {view}
              </button>
      ...
      {/* Week Calendar Board */}
      {currentView === 'Week' && (
        <div className="panel rounded-2xl border-slate-200 overflow-hidden shadow-2xl">
          {/* Days Names Bar */}
          <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-100 py-3 text-center">
            {weekDates.map((date, idx) => {
              const isToday = date.toDateString() === new Date().toDateString();
              return (
                <div key={idx} className="space-y-1 border-r border-slate-100 last:border-0">
                  <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">
                    {date.toLocaleDateString([], {weekday: 'short'})}
                  </p>
                  <p className={`text-base font-bold h-7 w-7 flex items-center justify-center mx-auto rounded-full ${
                    isToday ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-slate-800'
                  }`}>
                    {date.getDate()}
                  </p>
                </div>
          ...
          <div className="h-[480px] overflow-y-auto divide-y divide-slate-900/60">
            {["09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00"].map((timeHour) => (
              <div key={timeHour} className="grid grid-cols-7 min-h-[90px] relative hover:bg-white/10 transition-colors">
                ...
                {weekDates.map((date, dayIdx) => {
                  ...
                  return (
                    <div 
                      key={dayIdx} 
                      className="border-r border-slate-100 last:border-0 p-1.5 flex flex-col gap-1.5 relative min-h-[80px]"
                      ...
                      {hourRes.map(res => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="font-extrabold text-slate-900 truncate mt-1">{cust?.fullName || 'Walk-in Client'}</p>
      ...
      {currentView !== 'Week' && (
        <div className="card p-12 text-center border-slate-200 rounded-2xl">
          <Clock className="h-10 w-10 text-brand-600 mx-auto mb-3 animate-pulse-subtle" />
          <h4 className="text-lg font-bold text-slate-900 mb-2">{currentView} Scheduler View</h4>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg p-6 shadow-2xl relative animate-in zoom-in-95 duration-150">
                  ...
                  {formData.id && (
                    <>
                      <button
                        ...
                        className="bg-slate-100 border border-slate-300 text-slate-700 hover:text-slate-900 px-3 py-2 rounded-xl text-xs flex items-center gap-1"
